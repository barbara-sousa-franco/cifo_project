...
#
# All operators work on the normalized [0, 1] genome and rely on Triangle.__init__ to enforce the domain constraints
# (MAX_TRIANGLE_SIZE, ALPHA_MIN/MAX, non-degenerate vertices). Whenever a mutation/crossover produces a new gene
#  block, it must rebuild the Triangle via Triangle(repr=...) so the constraint repair runs on the result.
...
def _flat_genome(individual: Individual) -> np.ndarray:
    """Flatten an Individual's genome into a 1-D NumPy array of shape
    (N_TRIANGLES * GENES_PER_TRIANGLE,) by concatenating all gene values
    across all triangles. Required by distance-based operators (fitness
    sharing, restricted mating).

    Parameters:
        - individual (Individual): The individual whose genome to flatten.

    Returns:
        - np.ndarray: 1-D float64 array of all gene values in order.
    """
    return np.fromiter(
        # Generator: iterate over every triangle, then over every gene
        # within that triangle — produces all 1000 values in sequence.
        (g for tri in individual.repr for g in tri.repr),
        dtype=np.float64,
        # Providing count avoids a second pass to determine the length,
        # making np.fromiter allocate the output array in one shot.
        count=len(individual.repr) * GENES_PER_TRIANGLE,  
    )


def _genome_distance(a: Individual, b: Individual) -> float:
    """Normalised Euclidean distance between two individuals in genotype
    space. Dividing by sqrt(genome_length) keeps the scale independent
    of N_TRIANGLES so distance thresholds remain meaningful regardless
    of genome size.
    ...
    Returns:
        - float: Normalised Euclidean distance in [0, 1].
    """
    ga = _flat_genome(a)
    gb = _flat_genome(b)
    # np.linalg.norm computes sqrt(sum((ga - gb)^2)) — the standard Euclidean distance 
    # Dividing by sqrt(len(ga)) normalises for genome length.
...
def fitness_sharing_tournament(
    ...
    Parameters:
        ...
        - maximization (bool): If True, larger fitness is better.
        - tournament_size (int): Number of contenders drawn per tournament.
        - sigma_share (float): Niche radius in normalised genotype space.
            Individuals closer than sigma_share contribute to each other's
            crowding. Smaller values → more niches → more diversity pressure.
        - strength (float): Penalisation intensity. 0 disables sharing entirely.
    ...
    # Collect raw fitness for all individuals in the population
    raw = np.array([ind.fitness() for ind in population], dtype=np.float64)

    # Stack all flattened genomes into a (n, 1000) matrix so we can compute all pairwise distances in vectorised form.
    # Each row corresponds to an individual's genome, and each column corresponds to a gene across the population.
    ...
    for i in range(n):
        ...
        # Euclidean distance from individual i to every other individual.
        # Shape: (n,) — one distance per population member.
        ...
        # 1. Normalise the distances by the niche radius (sigma_share), values below 1 are within the niche
        # 2. Invert and clip to get sharing value: close neighbors have positive values, distant neighbors 
        # have zero contribution
        sharing = np.maximum(0.0, 1.0 - distances / max(1e-9, sigma_share))

        # High value -> many similar neighbours -> individual is in a dense niche.
    ...
    # Standard tournament selection on the adjusted scores.
...
def restricted_mating_selection(
    ...
    Parameters:
        - population (list[Individual]): Current population.
        - parent1 (Individual): The already-selected first parent.
        - maximization (bool): If True, larger fitness is better.
        - pool_size (int): Number of candidates to draw before filtering.
        - min_distance (float): Minimum allowed genotypic distance from parent1.
            Candidates closer than this are too similar (inbreeding risk).
        - max_distance (float): Maximum allowed genotypic distance from parent1.
            Candidates further than this are too different (disruption risk).
        - base_selection (callable, optional): Selection function used to sample
            candidates. Defaults to tournament_selection.
    ...
    # Draw pool_size candidates using the base selection function
    candidates = [base_selection(population, maximization) for _ in range(max(1, pool_size))]

    # Compute genotypic distance from parent1 for each candidate and filter to those within the 
    ...
    if valid:
        ...
        # Among valid candidates, pick the one with the best raw fitness: compatible distance AND best quality.
    ...
    # Fallback: no candidate fell inside [min_distance, max_distance].
    # Instead of giving up and picking randomly, we choose the candidate whose distance is closest to the 
    # midpoint of the allowed window, this still partially respects the intent of restricted mating
    # (avoiding the extremes) even when the ideal window is empty. Choosing closest to parent1 would risk 
    # inbreeding; choosing furthest would risk disruption, the midpoint is the least bad compromise.
...
def uniform_crossover(p1, p2, xo_prob, verbose=False, p=0.5, **kwargs):
    ...
    Parameters:
        ...
        - verbose (bool): If True, prints the resulting children.
        - p (float): Probability of inheriting each gene from p1 (default 0.5).

    Returns:
        - Tuple[Individual, Individual]: Two offspring.
    
    """
    if random.random() > xo_prob: # No crossover, return deep copies of the parents
...
def kpoint_crossover(p1, p2, xo_prob, verbose=False, k_min=3, k_max=7, **kwargs):
    ...
    Parameters:
        ...
        - xo_prob (float): Crossover probability.
        - verbose (bool): If True, prints the resulting children.
        - k_min (int): Minimum number of crossover points.
        - k_max (int): Maximum number of crossover points.
    ...
    # Sample K distinct cut points from {1, ..., size-1}. min() guards against k > size-1.
    cuts = sorted(random.sample(range(1, size), min(k, size - 1)))

    c1, c2 = [], []
    prev = 0

    # Iterate over segments defined by consecutive cut points.
    # cuts + [size] ensures the last segment (cut : end) is also processed.
    # Even-indexed segments: each child takes from its own parent.
    # Odd-indexed segments: each child takes from the opposite parent.
...
def reduced_surrogate_crossover(p1, p2, xo_prob, verbose=False, **kwargs):
    ...
    # All indices where the parents differ
    diff = [i for i in range(size) if r1[i] != r2[i]]

    if len(diff) < 2:  # Parents almost identical - no meaningful crossover points
        return deepcopy(p1), deepcopy(p2)
    
    # # Exclude the last differing position: cutting there would leave only
    # identical material after the cut, making the swap uninformative.
...
def shuffle_crossover(p1, p2, xo_prob, verbose=False, **kwargs):
    ...
    # Generate a random permutation of triangle positions and apply it
    # identically to both parents so the relative pairing is preserved.
    indices = list(range(size))
    random.shuffle(indices)
    s1 = [r1[i] for i in indices]
    s2 = [r2[i] for i in indices]

    # Single-point crossover on the shuffled genomes.
    cut = random.randint(1, size - 1)
    cs1 = s1[:cut] + s2[cut:]
    cs2 = s2[:cut] + s1[cut:]

    # Build the inverse permutation: inv[orig_pos] = shuffled_pos.
    # This maps each triangle back to its original position in the genome.
    inv = [0] * size
    for new_pos, orig_pos in enumerate(indices):
        inv[orig_pos] = new_pos

    # Apply the inverse permutation to restore the original genome order.
...
def adaptive_crossover_schedule(p1, p2, xo_prob, verbose=False,
    ...
    Parameters:
        ...
        - xo_prob (float): Crossover probability.
        - verbose (bool): If True, prints the resulting children.
        - current_gen (int): The current generation number (starting from 0).
        - max_gen (int): The total number of generations planned for the evolution process.

    Returns:
        Tuple[Individual, Individual]: Two offspring.

    """
    # Calculate the current phase of evolution as a fraction of the total generations.
    phase = current_gen / max_gen

    # Select the crossover method based on the current phase
    if phase < 0.5:
...
def triangles_overlap(t1, t2):
    """ Checks if two triangles overlap by comparing their bounding boxes axis-aligned bounding box (AABB) 
    intersection.
    ...
    """
    # Bounding box of the triangle 1: min/max of x and y across the 3 vertices.
...
def triangle_mutation_vcf(individual, mutation_prob, **kwargs):
    ...
    # Initial defensive copy of the individual's representation to ensure that we do not modify the original 
    # individual directly.
    ...
    for i, triangle in enumerate(new_repr):
        ...
        mutations = random.sample(["vertices", "color", "full", "order"], k=random.randint(1, 4))

        # If "full" was chosen, it doesn't make sense to apply the other mutations, so we can skip them
        # and directly replace the triangle with a new random one. The Triangle constructor will handle all the domain constraints.
        ...
        if "color" in mutations:
            # Perturb all four RGBA channels with Gaussian noise.
            for j in range(6, 10):
        ...
        if "order" in mutations:
            # Find triangles that overlap with this one and swap with one of them.
            # Only overlapping triangles are candidates because swapping non-overlapping triangles has no visual
            # effect.
...
def gaussian_gene_mutation(individual, mutation_prob, sigma=0.05, **kwargs):
    ...
    for triangle in individual.repr:
        ...
        # Perturb all 10 genes independently and clip to [0, 1].
        for k in range(GENES_PER_TRIANGLE):
            new_genes[k] = max(0.0, min(1.0, new_genes[k] + random.gauss(0.0, sigma)))

        # Rebuild Triangle to re-apply alpha clip, size shrink, and
        # degenerate vertex repair after the gene-level perturbation.
...
def color_creep_mutation(individual, mutation_prob, color_sigma=0.04, **kwargs):
    ...
    for triangle in individual.repr:
        ...
        # Each color gene mutates independently with probability mutation_prob.
        for k in range(6, 10):
...
def adaptive_mutation_schedule(individual, mut_prob, current_gen=0, max_gens=100, verbose=False, **kwargs):
    ...
    # Select the mutation method based on the current phase
    if phase < 0.40:
