...
# Domain constraints (see header). Values picked to roughly match the
# "minimalistic 100 triangles" target while still allowing one triangle to
# cover ~6% of the canvas at most (0.25 x 0.25 of a 300x400 image).
...
def shrink_to_max_size(genes: list) -> list:
...
class Triangle:
    ...
    def copy(self):
        # Bypass __init__ to skip the (already-satisfied) constraint repairs
        # on the hot path. The values in self.repr are guaranteed valid
        # because every constructor (and every operator that uses with_repr)
        # routes through Triangle(repr=...).
...
class Individual:
    ...
    antialiased : bool, default False
        Render with SUPERSAMPLE_FACTOR-x supersampling. Off by default so
        that hyperparameter sweeps remain tractable.
    """
    ...
    def render(self) -> Image.Image:
        ...
        single pass - no per-triangle layer is allocated. If antialiased,
        renders at SUPERSAMPLE_FACTOR-x resolution and downsamples with
        LANCZOS for smoother triangle edges.
        """
        if self.antialiased:
            ...
            for triangle in self.repr:
                # Decode vertices into the supersampled coordinate frame.
                g = triangle.repr
                verts = [
                    (round(g[0] * (w - 1)), round(g[1] * (h - 1))),
                    (round(g[2] * (w - 1)), round(g[3] * (h - 1))),
