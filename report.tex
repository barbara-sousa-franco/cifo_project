...
(population $500$, $15{,}000$ generations, $\sim\!28.6$~h of wall-clock)
reached a final RMSE of $\mathbf{17.12}$, the best image produced in
the project. As a Challenge extension, the perceptual CIEDE2000 metric
reached $\Delta E_{00} = 6.20$ on a shorter budget (POP $= 200$,
GENS $= 5{,}000$); cross-evaluation against the budget-matched
RMSE-tuned run is reported in \S7.
...
elitism on. The run completed in $\sim\!28.6$~h of wall-clock and
reached a final RMSE of $\mathbf{17.12}$ --- the best image produced
in the project (Figure~\ref{fig:final_run_best}).

\begin{figure}[h]
\centering
\includegraphics[width=0.65\linewidth]{figures/final_run_best.png}
\caption{Best individual from the long-budget final run
(POP $= 500$, GENS $= 15{,}000$) alongside the target. RMSE $= 17.12$,
$\Delta E_{00} = 5.46$ when re-evaluated under the perceptual metric.}
\label{fig:final_run_best}
\end{figure}

\textbf{Long vs short budget.} We also ran the same configuration
under a reduced budget (\texttt{final\_run\_short}: POP $= 200$,
GENS $= 5{,}000$, single seed), which reached RMSE $= 19.63$. The
long-budget run improves on the short-budget result by $2.51$ RMSE
($-12.8\%$) at a cost of roughly $10\times$ more wall-clock, and
on the 15-run calibration baseline (\texttt{sample\_11},
mean $= 26.56$) by $9.44$ RMSE ($-35.5\%$). The diminishing-returns
shape of the trade-off is expected: the short-budget setting already
captures the coarse structure of the painting, while the long budget
mostly refines colour gradients and edge placement. The statistical
claim of the project rests on the OFAT and random-search phases
($\sim\!100$ total runs); the two long-budget runs are illustrative
single-seed reconstructions, not estimates of expected performance.
...
compute budget. We therefore ran two GAs that share an identical
hyperparameter setup and budget (POP $= 200$, GENS $= 5{,}000$, single
seed) and differ \emph{only} in the fitness function:
\texttt{final\_run\_short} for the pixel-RMSE side and
\texttt{ciede2000\_short} for the perceptual side. Each best
individual was then scored under \emph{both} metrics
(Table~\ref{tab:cross_eval}); the side-by-side rendering is shown in
Figure~\ref{fig:rmse_vs_ciede2000}.
...
\gmid
RMSE-optimised      (\texttt{final\_run\_short})  & $\mathbf{19.63}$ & $6.09$ \\
...
Each row is one best individual scored under \emph{both} metrics; the
diagonal (in bold) is each GA's native objective and the off-diagonal
quantifies how each winner performs under the rival metric.}
\label{tab:cross_eval}
\end{table}

\begin{figure}[h]
\centering
\includegraphics[width=\linewidth]{figures/rmse_vs_ciede2000.png}
\caption{Best individual produced by each fitness regime under the
matched short budget. The RMSE-tuned image (centre) preserves edge
contrast; the CIEDE2000-tuned image (right) smooths skin tones and
saturates the turban hue. Both numbers per panel correspond to
Table~\ref{tab:cross_eval}.}
\label{fig:rmse_vs_ciede2000}
\end{figure}

\textbf{Reading of the result.} On its native pixel metric the
RMSE-tuned GA wins by $4.20$ RMSE points ($19.63$ vs $23.83$), as
expected. The more interesting observation is the right column: the
RMSE-tuned image also scores marginally lower $\Delta E_{00}$ ($6.09$
vs $6.20$) than the CIEDE2000-tuned image. At this short budget the
geometric placement of triangles dominates the fitness signal, and
the smoother RMSE landscape converges faster to a configuration that
also happens to be perceptually competitive. We expect the perceptual
advantage of CIEDE2000 to emerge more clearly at the full POP $= 500$,
GENS $= 15{,}000$ budget (estimated $\sim\!4$ days under
CIEDE2000), where edge placement is already converged and the
remaining optimisation budget is spent on colour --- which is exactly
the regime in which the two metrics differ most. We list that
extended Challenge run as future work.

\textbf{Visual reading.} Both reconstructions in
Figure~\ref{fig:rmse_vs_ciede2000} share gross structure (head,
turban silhouette, dark background). The CIEDE2000 image preserves
smoother skin tones and a more saturated turban hue --- the
ultramarine region where CIE76 historically fails --- while the RMSE
image sharpens local edges at the expense of overall colour balance,
consistent with what each metric numerically penalises.
...
reduction supported by 15-run statistical evidence. The long-budget
final run (POP $= 500$, GENS $= 15{,}000$) pushed this further to
RMSE $= 17.12$ on a single seed, the best image of the project. The
Challenge extension transferred the same pipeline cleanly to a
perceptual fitness ($\Delta E_{00} = 6.20$ on the short budget); the
budget-matched cross-evaluation revealed that, at this short setting,
the RMSE-tuned GA is competitive on both metrics, while the perceptual
advantage of CIEDE2000 is expected to materialise only at the full
long budget --- which exceeded our deadline.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{figures/mutation_comparison.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{figures/crossover_comparison.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{figures/probabilities_comparison.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{figures/triangle_comparison_graphs.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{figures/triangle_comparison_images.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{figures/diversity_images.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{figures/top3_graphs.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{figures/top3_images.png}
