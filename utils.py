...
def plot_best_individuals(
    ...
    Args:
        - best_inds: One of three shapes is accepted:
            * a single Individual (from run_single_experiment)
            * a dict mapping config name (str) or (mut_prob, xo_prob) tuple
              to the best Individual (from run_experiment)
            * a list of Individuals
        - target_img (PIL.Image or np.ndarray): The target image to display first.
        - configs (optional): Operator configs (each with key 'name') or
          (mut_prob, xo_prob) tuples. If None, the keys of ``best_inds`` (when
          it is a dict) are used as labels.
        - title_prefix (str): Prefix for the figure suptitle.
    """

    # --- Coerce best_inds into an ordered list of (label, Individual) ---
    if isinstance(best_inds, dict):
        if configs is None:
            entries = [(str(k), best_inds[k]) for k in best_inds]
        else:
            if configs and isinstance(configs[0], dict):
                entries = [(c["name"], best_inds[c["name"]]) for c in configs]
            else:
                entries = [(f"mut={k[0]} xo={k[1]}", best_inds[k]) for k in configs]
    elif isinstance(best_inds, list):
        entries = [(f"Run {i + 1}", ind) for i, ind in enumerate(best_inds)]
    else:
        # Single Individual
        entries = [("Best", best_inds)]

    keys   = [e[0] for e in entries]
    labels = keys
    inds_by_key = {e[0]: e[1] for e in entries}

    fig, axes = plt.subplots(1, len(keys) + 1, figsize=(4 * (len(keys) + 1), 4))
    # plt.subplots collapses to a 0-d array when there is only one subplot
    if len(keys) == 0:
        axes = [axes]
    ...
    for ax, key, label in zip(axes[1:], keys, labels):
        ...
        ax.set_title(f"{label}\nfitness={ind.fitness():.2f}", fontsize=10)
        ax.axis("off")
